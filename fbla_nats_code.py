...
import io        # (currently unused) in-memory byte stream helper
import json      # reading/writing the user_data.json save file
import math      # sin()/pi used to synthesize sound wave samples
import os        # building file paths and checking if files exist
import random    # shuffling answer options and picking random questions
import sys       # sys.exit() to cleanly close the program
import array     # efficient typed array used to build raw audio sample buffers
import pygame    # the game engine: window, drawing, input, audio


# Configure the audio mixer (sample rate, bit depth, channels, buffer size)
# BEFORE pygame.init() so these custom settings actually take effect -
# pygame locks the mixer's settings once it has already been initialized.
pygame.mixer.pre_init(44100, -16, 1, 512)
pygame.init()  # Initialize all pygame subsystems (display, mixer, fonts, etc.)

# Core window settings used throughout the game.
WIDTH, HEIGHT = 800, 500                          # Window dimensions in pixels
screen = pygame.display.set_mode((WIDTH, HEIGHT)) # The main drawable surface/window
pygame.display.set_caption("2000's Trivia Challenge")  # Window title bar text
...
DATA_FILE = os.path.join(os.path.dirname(__file__), "user_data.json")  # Save file for accounts/high scores
IMAGE_DIR = os.path.join(os.path.dirname(__file__), "images")          # Secondary folder checked for question images
PICTURES_DIR = os.path.join(os.path.dirname(__file__), "pictures")     # Primary folder checked for question images
FALLBACK_PICTURES_DIR = os.path.join(    # Backup location to look for images if not found above
    os.path.expanduser("~"),
    "Desktop",
...
BLACK = (0, 0, 0)              # Pure black; default text/feedback color
BLUE = (100, 180, 255)         # Sign-in/primary action buttons, dark-mode button bg
GREEN = (80, 200, 120)         # Positive/"correct" actions (register, correct answer highlight)
RED = (255, 0, 0)              # Negative/"back" or wrong-answer highlight
GRAY = (220, 220, 220)         # Neutral/"skip" button color
PURPLE = (201, 185, 255)       # Accent color (dark-mode Next button, decorative circles)
LIGHT_BLUE = (173, 216, 230)   # Decorative backdrop circles / confetti
LIGHT_GREEN = (144, 238, 144)  # Decorative backdrop circles / score badges
NAVY = (15, 22, 52)            # (currently unused) dark navy accent
GOLD = (240, 190, 60)          # Guest button, card borders, decorative highlights
LIGHT = (235, 236, 242)        # Fallback input-box background color
PINK = (255, 192, 203)         # Confetti accent color
BLUE_OPTION = (70, 120, 255)   # Background color for the 4 answer-choice buttons
...
title_font = pygame.font.SysFont("comic sans ms", 36, bold=True)     # Screen titles/headers
question_font = pygame.font.SysFont("comic sans ms", 20)             # Question text, score lines
option_font = pygame.font.SysFont("comic sans ms", 24)               # Button labels, input box text
feedback_font = pygame.font.SysFont("comic sans ms", 30, bold=True)  # "Correct!"/"Wrong!" feedback
small_font = pygame.font.SysFont("comic sans ms", 16)                # Fine print, fact bullets, player name
prompt_font = pygame.font.SysFont("comic sans ms", 22, bold=True)    # Bottom-of-screen prompts/progress label
...
THEMES = {
    "light": {
        ...
        "surface": (245, 245, 245),      # Slightly off-white background used on the gameplay screen
        "text": BLACK,                   # Default body text color
        "title": BLACK,                  # Big title/header text color
        "button": (220, 220, 220),       # Default (unselected) button background
        "button_selected": (80, 80, 80), # Button background when selected/active
        "button_text": BLACK,            # Text color drawn on top of buttons
        "input_bg": WHITE,                # Username/password input box background
        "input_border": (60, 60, 60),    # Input box border color
        "input_text": BLACK,             # Text typed into input boxes
        "accent": (60, 60, 60),          # Secondary/muted text (subtitles, prompts)
        "feedback_shadow": (200, 200, 200),  # Drop-shadow color behind "Correct!"/"Wrong!" text
    },
    "dark": {
        "background": BLACK,             # Main screen background
        "surface": (30, 30, 30),         # Slightly lighter-than-black background for gameplay screen
        "text": WHITE,                   # Default body text color
        "title": WHITE,                  # Big title/header text color
        "button": (100, 180, 255),       # Default (unselected) button background
        "button_selected": (200, 200, 200),  # Button background when selected/active
        "button_text": BLACK,            # Text color drawn on top of buttons
        "input_bg": (40, 40, 40),        # Username/password input box background
        "input_border": (180, 180, 180), # Input box border color
        "input_text": WHITE,             # Text typed into input boxes
        "accent": (200, 200, 200),       # Secondary/muted text (subtitles, prompts)
        "feedback_shadow": (50, 50, 50), # Drop-shadow color behind "Correct!"/"Wrong!" text
    }
}
...
celebration_sound = create_celebration_sound()    # Played on the post-round score screen
correct_sound = create_correct_sound()            # Played when an answer is correct
wrong_sound = create_wrong_sound()                # Played when an answer is wrong
login_intro_sound = create_login_intro()          # Played once when the login screen first appears
background_music = create_background_music()      # Looping music for login/home screens
yay_sound = create_yay_sound()                    # Played on the final summary (all categories done) screen

# One-shot flags so intro/celebration sounds only play once per screen
# visit instead of replaying every frame.
celebration_played = False         # True once celebration_sound has played for the current round-over screen
login_intro_played = False         # True once login_intro_sound has played this app run
background_music_playing = False   # True while background_music is currently looping/playing
yay_played = False                 # True once yay_sound has played for the current final summary screen
music_muted = False                # True if the player has muted background music via the toggle button
...
class InputBox:
    ...
    def draw(self, surface, theme=None):
        """Render the input box: background, border (with an optional
        gold blink hint), typed/placeholder text, and a blinking text
        cursor while the box has focus."""
        ...
        now = pygame.time.get_ticks()  # Current time in ms, used to drive blink animations
...
auth = AuthManager(DATA_FILE)  # Loads (or creates) the JSON-backed account store
current_user = "Guest"         # Name of the currently logged-in player ("Guest" if not signed in)
...
game_state = "login"
auth_message = "Sign in or play as a guest player!"  # Status/error text shown under the login form

# Input fields and system buttons
username_box = InputBox((220, 180, 360, 45), "Username")                    # Username text field on login screen
password_box = InputBox((220, 240, 360, 45), "Password",is_password=True)   # Masked password text field on login screen
...
facts_view_category = None        # Category whose facts are currently being displayed
facts_return_state = "bonus_offer"  # game_state to switch back to when the facts Back button is clicked
...
final_summary_confetti = []  # Cached list of confetti pieces for the final summary screen (currently unused; generated on demand via create_final_summary_confetti)

# Prevent immediate mouse-click propagation when switching screens
ignore_mouse = False  # (currently unused) flag reserved for suppressing a stray click after a screen change

feedback = ""                  # Bottom-of-screen feedback text ("Correct!"/"Wrong!"), empty when none to show
feedback_color = BLACK         # Color to render `feedback` in (green for correct, red for wrong)
bottom_question_prompt = "Please choose one of the answers above"  # Default hint shown before feedback appears
...
class Button:
    ...
    def __init__(self, x, y, w, h, text, color=None, enabled=True):
        """Create a button at (x, y) with size (w, h) and the given label.
        color: optional fixed background color that overrides theme colors.
        enabled: if False, the button is grayed out and not clickable."""
        self.rect = pygame.Rect(x, y, w, h)  # Position + size, also used for click hit-testing
        self.text = text                     # Label drawn on the button (also used as an identifier in places)
        self.color = color  # Custom color override
        self.enabled = enabled               # Whether the button can currently be clicked

    def get_text_color(self, bg_color):
        """Return correct text color based on your rules."""
        # Backgrounds light/bright enough for black text to stand out clearly;
        # anything else (e.g. dark backgrounds) falls back to white text.
    ...
    def draw(self, surface, selected=False, theme=None):
        """Draw this button onto `surface`. `selected` swaps in the theme's
        button_selected color (used to highlight the chosen answer).
        `theme` defaults to the globally active light/dark theme dict."""
        if theme is None:
            theme = THEMES[theme_mode]
...
def create_buttons():
    ...
    buttons = []  # Will hold the 4 Button objects, one per answer choice
    for i in range(4):
        # Stack the 4 buttons vertically 60px apart; load_question() repositions
        # them later based on where the question text ends.
        buttons.append(Button(200, 150 + i * 60, 400, 45, ""))
    return buttons

buttons = create_buttons()  # The 4 on-screen answer-choice buttons (shared/reused across questions)
next_button = Button(620, 420, 150, 50, "NEXT")                       # Advances to the next question during gameplay
theme_toggle_btn = Button(WIDTH - 180, 20, 160, 40, "Dark Mode")      # Top-right: switches light/dark theme (label flips dynamically)
music_toggle_btn = Button(20, 20, 160, 40, "Mute Music")              # Top-left: mutes/unmutes background music (label flips dynamically)
main_menu_button = Button(220, 320, 170, 45, "Continue", BLUE)        # Round-over screen: go to next category or final summary (label set dynamically)
game_over_facts_btn = Button(410, 320, 170, 45, "View Facts", GREEN)  # Round-over screen: view fun facts about the just-played category
final_summary_menu_btn = Button(315, 430, 170, 45, "Back to Login", BLUE)  # Final summary screen: log out and return to login
bonus_round_button = Button(410, 320, 170, 45, "Bonus 3", GREEN)      # (currently unused leftover button)

# Login system controls
login_btn = Button(220, 310, 170, 45, "Sign In", BLUE)        # Authenticate with the typed username/password
register_btn = Button(410, 310, 170, 45, "Register", GREEN)   # Create a new account from the typed username/password
guest_btn = Button(220, 370, 360, 45, "Play as Guest", GOLD)  # Skip login and play without saving a high score

# Home / Tutorial buttons (shown after successful login)
# Positioned higher so they don't overlap login buttons
home_start_btn = Button(220, 200, 170, 45, "Start Quiz", BLUE)     # Go to the category-selection screen
home_tutorial_btn = Button(410, 200, 170, 45, "Tutorial", GREEN)  # Go to the how-to-play tutorial screen
home_logout_btn = Button(220, 260, 360, 45, "Log Out", GOLD)      # Sign out and return to the login screen

# Category-selection screen buttons
category_tech_btn = Button(180, 180, 260, 60, "Technology", BLUE)             # Start the Technology category quiz
category_celebrity_btn = Button(180, 260, 260, 60, "Celebrities & More", GREEN)  # Start the Celebrities & More category quiz
category_history_btn = Button(180, 340, 260, 60, "Historical Events", GOLD)   # Start the Historical Events category quiz
category_back_btn = Button(460, 420, 260, 60, "Back", RED)                    # Return to the home screen

# Bonus-offer / facts-view screen buttons
bonus_question_button = Button(180, 320, 260, 55, "Bonus Question", BLUE)  # Accept the optional bonus question
skip_bonus_button = Button(460, 320, 260, 55, "Skip Bonus", GRAY)          # Decline the bonus question and end the round
facts_view_button = Button(315, 385, 170, 45, "View Facts", GREEN)        # Open the fun-facts screen from the bonus offer
facts_back_button = Button(WIDTH // 2 - 85, HEIGHT - 70, 170, 45, "Back", BLUE)  # Return from the facts screen to where it was opened

# Tutorial screen buttons (different positions)
tutorial_start_btn = Button(220, 360, 170, 45, "Start Quiz", BLUE)  # Skip the tutorial and go straight to category selection
tutorial_back_btn = Button(410, 360, 170, 45, "Back", GREEN)        # Return to the home screen


# Total number of "base" (non-bonus) questions across a full session:
# 5 questions per category times the number of categories.
TOTAL_BASE_QUESTIONS = len(question_categories) * 5                       # e.g. 3 categories * 5 = 15
TOTAL_MAX_QUESTIONS = TOTAL_BASE_QUESTIONS + len(question_categories)     # Base questions + 1 possible bonus per category
final_win_image = load_image("win.png", max_size=(220, 160))  # Trophy/win artwork shown on the final summary screen
...
def reset_quiz_progress():
    ...
    completed_categories = set()       # Forget which categories were already played
    session_base_score = 0             # Reset cumulative base-question score
    current_category_base_score = 0    # Reset the in-progress category's score
    bonus_points = 0                   # Reset cumulative bonus-question score
    session_base_answered = 0          # Reset count of base questions answered
    session_bonus_answered = 0         # Reset count of bonus questions answered
...
def start_game(question_list, starting_score=0, starting_total=None, reset_bonus=True, question_count=None, is_bonus=False):
    ...
    if reset_bonus:
        bonus_used = False    # Player hasn't accepted/declined a bonus yet for this round
        bonus_taken = False   # Player hasn't accepted a bonus yet
        bonus_offer = False   # Not currently showing the bonus-offer screen
    current_is_bonus = is_bonus  # Whether this round is a single bonus question vs. a 5-question category
    ...
    current_questions = build_quiz_questions(question_list, question_count)  # Freshly shuffled question set
    question_index = 0          # Start at the first question
    score = starting_score       # Running correct-answer count for this round
    total_questions = len(current_questions) if starting_total is None else starting_total + len(current_questions)
    selected_option = None       # No answer chosen yet
    game_over = False            # Round is actively in progress
    feedback = ""                # Clear any leftover "Correct!"/"Wrong!" text
    feedback_color = BLACK
    load_question()              # Populate the answer buttons for question #1
...
def start_category(category_name):
    ...
    selected_category = category_name   # Remember which category is being played
    last_completed_category = None       # Clear any stale "last completed" reference
    start_game(question_categories[category_name], question_count=5, is_bonus=False)
    game_state = "game"  # Switch to the gameplay screen
...
def start_bonus_question():
    ...
    used_questions = {q["question"] for q in current_questions}  # Questions already seen this round
    ...
    if not available_bonus:
        # Edge case: every question in the category was already used as a
        # base question, so there's nothing left to ask as a bonus.
        bonus_offer = False
        game_over = True
        auth.save_high_score(current_user, score)
        return

    bonus_q = random.choice(available_bonus)  # Pick one unseen question at random
    current_questions = [{
        "question": bonus_q["question"],
        "options": random.sample(bonus_q["options"], len(bonus_q["options"])),  # Shuffle answer order
        "answer": bonus_q["answer"],
        "image": bonus_q.get("image")
    }]
    question_index = 0          # The bonus question is the only item in current_questions
    total_questions += 1        # Count it toward the round's denominator
    selected_option = None
    feedback = ""
    feedback_color = BLACK
    bonus_used = True           # Bonus offer has now been resolved
    bonus_taken = True          # Player chose to attempt it
    bonus_offer = False         # No longer showing the offer screen
    current_is_bonus = True     # Marks this question as the bonus (for scoring/labels)
    game_state = "game"         # Show the gameplay screen for the bonus question
    game_over = False
    load_question()
...
def open_facts_view(category_name, return_state="bonus_offer"):
    ...
    facts_view_category = category_name  # Which category's facts to display
    facts_return_state = return_state     # Where the Back button should take the player
...
def load_question():
    ...
    selected_option = None  # No answer picked yet for this question
    q = current_questions[question_index]  # The question dict to display
    ...
    for i in range(4):
        buttons[i].text = q["options"][i]  # Fill in this question's (already-shuffled) answer text
        buttons[i].color = None             # Clear any green/red highlight left from the previous question
...
def next_question():
    ...
    question_index += 1  # Move to the next question in current_questions
    feedback = ""         # Clear the previous question's "Correct!"/"Wrong!" text

    if question_index >= len(current_questions):  # Ran out of questions in this round
        ...
        if not bonus_taken and not bonus_offer:
            session_base_score += current_category_base_score  # Bank this category's points into the session total
            last_completed_category = selected_category
            bonus_offer = True       # Show the bonus-offer screen next
            game_over = False
            selected_option = None
        ...
        if not current_is_bonus:
            session_base_score += current_category_base_score  # Bank points (covers the "skip bonus" path)
            completed_categories.add(selected_category)          # Mark the category as finished
            last_completed_category = selected_category
        game_over = True  # Show the round-over score screen
        auth.save_high_score(current_user, score)
    else:
        load_question()  # Still more questions left in this round
...
def draw():
    ...
    if game_state == "login":
        ...
        # ---------------- GAMEPLAY SCREEN (active question or round-over) ----------------
        ...
        if not game_over:
            # --- Showing an active question ---
            q = current_questions[question_index]  # The question currently being shown
            ...
            # --- Round is over: show the round score summary ---
        ...
        # ---------------- FINAL SUMMARY SCREEN (all categories complete) ----------------
        global yay_played
        if not yay_played:
...
running = True  # Controls the main game loop; set to False to exit cleanly

while running:
    draw()  # Render the current screen every frame, before handling input

    for event in pygame.event.get():  # Process every queued input/window event this frame
        ...
        if game_state == "login":
            # ---------------- LOGIN SCREEN EVENTS ----------------
            username_box.handle_event(event)  # Let the username field handle typing/focus
            password_box.handle_event(event)  # Let the password field handle typing/focus

            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()

                if theme_toggle_btn.is_clicked(pos):
                    theme_mode = "light" if theme_mode == "dark" else "dark"  # Flip the active theme
                    theme_toggle_btn.text = "Dark Mode" if theme_mode == "light" else "Light Mode"
                    continue

                if music_toggle_btn.is_clicked(pos):
                    music_muted = not music_muted  # Toggle mute state; draw() reacts to this each frame
                    continue
                ...
                if login_btn.is_clicked(pos):
                    ...
                    if success:
                        reset_quiz_progress()  # Start this session's progress tracking from scratch
                        # theme_mode = "light"
                        # theme_toggle_btn.text = "Dark Mode"
                        current_user = username
                        game_state = "home"  # Move to the home/welcome screen
                        print(f"DEBUG: login successful for {username}; set game_state=home")
                        pygame.event.clear()   # Drop any queued events so a stray click doesn't carry over
                        pygame.time.wait(80)
                        continue
                    ...
                    if success:
                        username_box.text = ""              # Clear the form
                        password_box.text = ""
                        username_box.active = True           # Focus the username field for sign-in
                        password_box.active = False
                        username_box.trigger_hint_blink()     # Draw attention to the fields
                    ...
                    # theme_mode = "light"
                    # theme_toggle_btn.text = "Dark Mode"
                    current_user = "Guest"  # Play without a saved account
                    game_state = "home"
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if home_start_btn.is_clicked(pos):
                    ...
                    game_state = "login"
                    homepage_intro_played = False  # NOTE: unused/undefined elsewhere; has no effect
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if category_tech_btn.is_clicked(pos):
                    start_category("Technology")        # Begin a 5-question Technology round
                elif category_celebrity_btn.is_clicked(pos):
                    start_category("Celebrities & More")  # Begin a 5-question Celebrities round
                elif category_history_btn.is_clicked(pos):
                    start_category("Historical Events")   # Begin a 5-question History round
                elif category_back_btn.is_clicked(pos):
                    game_state = "home"  # Cancel category selection, return home
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if tutorial_start_btn.is_clicked(pos):
                    go_to_category_select()  # Skip ahead straight to choosing a category
                elif tutorial_back_btn.is_clicked(pos):
                    game_state = "home"

        elif game_state == "game" and not game_over:
            # ---------------- ACTIVE QUESTION EVENTS ----------------
            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                ...
                for btn in buttons:
                    if btn.is_clicked(pos):
                        selected_option = btn.text  # Record which answer the player picked (highlighted on next draw)

                if next_button.is_clicked(pos) and selected_option:
                    check_answer()        # Score the pick and color-code correct/wrong buttons
                    draw()                  # Force an immediate redraw so the feedback is visible...
                    pygame.time.delay(1200)  # ...and pause briefly so the player can read it
                    next_question()         # Then advance to the next question (or end the round)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if bonus_question_button.is_clicked(pos):
                    start_bonus_question()  # Player wants to attempt the bonus question
                elif skip_bonus_button.is_clicked(pos):
                    # Player declined the bonus: end the round immediately.
                    bonus_offer = False
                    bonus_used = True
                    game_over = True
                    game_state = "game"  # Show the round-over score screen
                    auth.save_high_score(current_user, score)

        elif game_state == "facts_view":
            # ---------------- FUN FACTS SCREEN EVENTS ----------------
            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                ...
                if facts_back_button.is_clicked(pos):
                    game_state = facts_return_state  # Go back wherever the facts screen was opened from

        elif game_state == "game" and game_over:
            # ---------------- ROUND-OVER SCREEN EVENTS ----------------
            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                ...
                if main_menu_button.is_clicked(pos):
                    # "Continue Playing" / "Back to Login" button (label is set
                    # dynamically in draw() based on whether categories remain).
                    if selected_category:
                        completed_categories.add(selected_category)  # Mark this category as finished
                    if len(completed_categories) == len(question_categories):
                        game_state = "final_summary"  # All categories done: show the overall summary
                    else:
                        go_to_category_select()        # Otherwise let the player pick another category
                    # Reset per-round state so the next round starts clean.
                    game_over = False
                    current_questions = []
                    total_questions = 0
                    bonus_used = False
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if final_summary_menu_btn.is_clicked(pos):
                    # Player is done: fully reset session state and sign out
...
# Clean shutdown once the main loop exits (running set to False).
pygame.quit()
sys.exit()
