...
class Button:
    ...
    def draw(self, surface, selected=False):
        if not self.enabled:
            ...
            text_color = BLACK if not self.custom_color or self.custom_color == GOLD else WHITE
...
def draw():
    ...
    if game_state == "login":
        ...
        screen.fill(NAVY)
        title = title_font.render("Choose a Quiz Category", True, GOLD)
        screen.blit(title, (WIDTH // 2 - title.get_width() // 2, 40))

        if completed_category:
            prompt = question_font.render("Choose a different category:", True, WHITE)
        else:
            prompt = question_font.render("Select one of the categories below:", True, WHITE)
        ...
        category_tech_btn.draw(screen)
        category_celebrity_btn.draw(screen)
        category_history_btn.draw(screen)
        category_back_btn.draw(screen)
        ...
        screen.fill(PURPLE)
        title = title_font.render("Bonus Question Offer", True, BLACK)
        screen.blit(title, (WIDTH // 2 - title.get_width() // 2, 40))
        ...
        bonus_question_button.draw(screen)
        skip_bonus_button.draw(screen)
